...
<?php
  ...
  <?php }else if ($url === "event" && $mode === "update") {
    ...
    ?>
    <h1>Create event</h1>
    <form action="index.php?url=update&id=<?php echo $eventId; ?>" method="post">
      <div class="form-group">
          <label for="inputEventName">Name of event</label>
  ...
  <?php }else{
    ...
    <div class="event-tools">
      <a class="btn btn-primary" href="index.php?url=event&mode=create" role="button">Create Event</a>
    </div>
  ...
  <?php while($row = mysql_fetch_assoc($events)){ ?>
       <tr>
           ...
           <td><a href="index.php?url=event&mode=remove&id=<? echo $row['id']; ?>">Remove</a></td>
       </tr>
       <?  } ?>
    </table>
 <?php } ?>
